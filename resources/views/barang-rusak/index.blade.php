...
@section('content')
    <div class="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-2 mb-4">
        ...
        <div class="d-flex flex-wrap gap-2">
            <a href="{{ route('barang-rusak.scan') }}" class="btn btn-info"><i class="bi bi-qr-code-scan me-1"></i>Scan
                QR</a>
            <a href="{{ route('barang-rusak.create') }}" class="btn btn-primary"><i
                ...
                <table id="dataTable" class="table table-hover" style="width:100%">
                    ...
                    <tbody>
                        ...
                        @foreach($barangRusak as $item)
                            <tr>
                                <td data-order="{{ $item->tanggal_rusak->format('Y-m-d') }}">
                                ...
                                <td>
                                    <span class="badge bg-danger">{{ $item->jumlah ?? 1 }} unit</span>
                                </td>
                                ...
                                <td>
                                    <div class="d-flex gap-1">
                                        <button type="button" class="btn btn-sm btn-warning" data-bs-toggle="modal"
                                            data-bs-target="#modalStatus{{ $item->id }}" title="Update Status">
                                            <i class="bi bi-pencil-square"></i>
                                        </button>
                                        <form action="{{ route('barang-rusak.destroy', $item) }}" method="POST"
                    ...
                    <form action="{{ route('barang-rusak.update-status', $item) }}" method="POST">
                        ...
                        <div class="modal-body">
                            <div class="alert alert-info mb-3">
                                ...
                                <small>Kerusakan: {{ $item->jenis_kerusakan }}</small><br>
                                <small class="text-primary"><strong>Total Rusak: {{ $item->jumlah ?? 1 }} unit</strong></small>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Jumlah yang Diupdate <span class="text-danger">*</span></label>
                                <input type="number" name="jumlah_update" class="form-control" min="1"
                                    max="{{ $item->jumlah ?? 1 }}" value="{{ $item->jumlah ?? 1 }}" required>
                                <small class="text-muted">Masukkan jumlah unit yang ingin diupdate statusnya (dari
                                    {{ $item->jumlah ?? 1 }} unit)</small>
                            </div>
                            <div class="mb-3">
                                ...
                                <select name="status" class="form-select" required>
                                    ...
                                    <option value="diproses" {{ ($item->status ?? '') == 'diproses' ? 'selected' : '' }}>Sedang
                                        Diproses</option>
                                    <option value="diperbaiki" {{ ($item->status ?? '') == 'diperbaiki' ? 'selected' : '' }}>Sudah
                                        Diperbaiki</option>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="catatan_status" class="form-control" rows="3"
                                    placeholder="Catatan perbaikan...">{{ $item->catatan_status }}</textarea>
                            </div>
                        </div>
