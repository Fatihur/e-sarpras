    ...
    <div class="card-body">
        <form method="POST" action="{{ route('gedung.update', $gedung) }}">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="nama_gedung" class="form-control @error('nama_gedung') is-invalid @enderror" value="{{ old('nama_gedung', $gedung->nama_gedung) }}" required>
                @error('nama_gedung')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>
            <div class="mb-3">
                <label class="form-label">Alamat</label>
                <textarea name="alamat_gedung" class="form-control" rows="2">{{ old('alamat_gedung', $gedung->alamat_gedung) }}</textarea>
            </div>
            <div class="mb-3">
                ...
                <select name="lahan_id" class="form-select">
                    ...
                    @foreach($lahan as $l)
                    <option value="{{ $l->id }}" {{ old('lahan_id', $gedung->lahan_id) == $l->id ? 'selected' : '' }}>{{ $l->nama_lahan }}</option>
                    @endforeach
                </select>
            </div>
