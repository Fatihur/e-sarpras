...
html5QrCode.start({ facingMode: "environment" }, { fps: 10, qrbox: 250 }, onScanSuccess).catch(err => {
    document.getElementById('reader').innerHTML = '<div class="alert alert-warning">Kamera tidak tersedia. Gunakan input manual.</div>';
});
...
function fetchBarang(kode) {
    document.getElementById('hasilScan').innerHTML = '<div class="text-center py-4"><div class="spinner-border text-primary"></div></div>';
        ...
        if (data.success) {
            ...
            document.getElementById('hasilScan').innerHTML = `
                ...
                <table class="table table-sm">
                    <tr><th width="120">Kode</th><td><code>${b.kode_barang}</code></td></tr>
            ...
            document.getElementById('hasilScan').innerHTML = `<div class="alert alert-danger"><i class="bi bi-x-circle me-2"></i>${data.message}</div>`;
        }
        setTimeout(() => { try { html5QrCode.resume(); } catch(e) {} }, 3000);
    });
}
</script>
@endpush
